...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql://postgres:ZCf%26Bz%2B68XFL%2F2%3F@db.cizzghxklafnuqzpevaq.supabase.co:5432/postgres",
)
...
class _DBConn:
    """Context manager: acquire → use → commit/rollback → release."""
...
def init_db():
    with _DBConn() as conn:
        with conn.cursor() as cur:
            ...
            """)
            # Safe migration: add is_banned if missing (idempotent)
            cur.execute("""
                DO $$
                BEGIN
                    IF NOT EXISTS (
...
def db_update_user(user_id: int, **kwargs):
    ...
    with _DBConn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                f"UPDATE users SET {fields} WHERE user_id = %s",
                values,
            )


def db_get_top_referrers(limit: int = 10) -> list:
    with _DBConn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
...
def db_create_withdrawal(
    user_id: int, amount: int, method: str, number: str
) -> int:
    with _DBConn() as conn:
        with conn.cursor() as cur:
            cur.execute(
...
def db_get_withdrawal(w_id: int) -> dict | None:
    with _DBConn() as conn:
        with conn.cursor() as cur:
...
def db_get_activation(a_id: int) -> dict | None:
    with _DBConn() as conn:
        with conn.cursor() as cur:
...
def get_state(user_id: int) -> tuple[str | None, str | None]:
    with _DBConn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT state, data FROM user_states WHERE user_id = %s",
                (user_id,),
            )
            row = cur.fetchone()
...
# ── Withdrawal/activation count helpers ───────
...
def kb_inline_approve_reject(
    action: str, record_id: int
) -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        types.InlineKeyboardButton(
            "✅ Approve", callback_data=f"{action}_approve_{record_id}"
        ),
        types.InlineKeyboardButton(
            "❌ Reject", callback_data=f"{action}_reject_{record_id}"
        ),
    )
    return kb
...
def handle_activate(msg: types.Message):
    ...
    if user["is_activated"]:
        bot.send_message(
            msg.chat.id, "✅ Your account is already activated!", reply_markup=kb_main()
        )
        return
...
def handle_text(msg: types.Message):
    ...
    if state == "awaiting_number":
        ...
        logger.info(
            "Withdrawal #%s: user %s, amount %s TK via %s",
            w_id, user_id, amount, method,
        )
...
def cmd_add_balance(msg: types.Message):
    ...
    try:
        ...
        bot.send_message(
            msg.chat.id,
            f"✅ Added {amount} TK to user {uid}. New balance: {new_bal} TK",
        )
        safe_send(
            uid,
            f"💰 Admin added <b>{amount} TK</b> to your account. Balance: <b>{new_bal} TK</b>",
        )
...
def cmd_ban(msg: types.Message):
    ...
    if not user:
        bot.send_message(
            msg.chat.id,
            f"❌ User <b>{target_username}</b> not found.",
            parse_mode="HTML",
        )
        return
    ...
    if user["is_banned"]:
        bot.send_message(
            msg.chat.id,
            f"⚠️ User <b>{target_username}</b> is already banned.",
            parse_mode="HTML",
        )
        return
...
def cmd_unban(msg: types.Message):
    ...
    if not user["is_banned"]:
        bot.send_message(
            msg.chat.id,
            f"⚠️ User <b>{target_username}</b> is not banned.",
            parse_mode="HTML",
        )
        return
...
def cmd_add(msg: types.Message):
    ...
    if len(parts) < 3:
        bot.send_message(
            msg.chat.id, "Usage: /add @username &lt;amount&gt;", parse_mode="HTML"
        )
        return
    ...
    logger.info(
        "Admin added %s TK to user %s. New balance: %s",
        amount, user["user_id"], new_bal,
    )
...
def cmd_remove(msg: types.Message):
    ...
    if len(parts) < 3:
        bot.send_message(
            msg.chat.id, "Usage: /remove @username &lt;amount&gt;", parse_mode="HTML"
        )
        return
    ...
    logger.info(
        "Admin removed %s TK from user %s. New balance: %s",
        amount, user["user_id"], new_bal,
    )
...
def cmd_broadcast(msg: types.Message):
    ...
    logger.info("Broadcast sent to %s/%s users (failed: %s)", sent, total, failed)
...
def cmd_msg(msg: types.Message):
    ...
    if len(parts) < 3:
        bot.send_message(
            msg.chat.id, "Usage: /msg @username &lt;message&gt;", parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        logger.warning(
            "Failed to send admin message to user %s: %s", user["user_id"], exc
        )
