...
def init_db():
    with _db_lock, PooledConnection() as conn:
        cur = conn.cursor()
        cur.execute("""
...
def db_update_withdrawal(w_id: int, status: str):
    with _db_lock, PooledConnection() as conn:
        cur = conn.cursor()
        cur.execute(
            "UPDATE withdrawals SET status = %s, resolved_at = to_char(NOW(), 'YYYY-MM-DD HH24:MI:SS') WHERE id = %s",
            (status, w_id),
        )
...
def db_create_activation(user_id: int) -> int:
    with _db_lock, PooledConnection() as conn:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO activations (user_id) VALUES (%s) RETURNING id", (user_id,)
        )
        return cur.fetchone()[0]
...
def db_update_activation(a_id: int, status: str):
    with _db_lock, PooledConnection() as conn:
        cur = conn.cursor()
        cur.execute(
            "UPDATE activations SET status = %s, resolved_at = to_char(NOW(), 'YYYY-MM-DD HH24:MI:SS') WHERE id = %s",
            (status, a_id),
        )
...
def set_state(user_id: int, state: str, data: str = ""):
    with _db_lock, PooledConnection() as conn:
        cur = conn.cursor()
        cur.execute(
            """INSERT INTO user_states (user_id, state, data) VALUES (%s, %s, %s)
...
def get_state(user_id: int) -> tuple:
    with _db_lock, PooledConnection() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT state, data FROM user_states WHERE user_id = %s", (user_id,)
        )
        row = cur.fetchone()
...
def db_count_withdrawals(user_id: int) -> int:
    with _db_lock, PooledConnection() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT COUNT(*) FROM withdrawals WHERE user_id = %s AND status = 'approved'",
            (user_id,),
        )
        row = cur.fetchone()
...
def db_count_pending_activations(user_id: int) -> int:
    with _db_lock, PooledConnection() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT COUNT(*) FROM activations WHERE user_id = %s AND status = 'pending'",
            (user_id,),
        )
        row = cur.fetchone()
        return row[0] if row else 0
